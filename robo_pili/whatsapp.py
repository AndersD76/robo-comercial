# -*- coding: utf-8 -*-
"""
Automação WhatsApp Web v3 - Pili Equipamentos
...
class WhatsAppBot:
    ...
    async def _aguardar_conexao(self, timeout=180):
        ...
        for i in range(timeout):
            ...
            if i % 15 == 0 and i > 0:
                print(
                    f"[WA/Pili {_ts()}] ℹ Aguardando QR Code... ({i}s)",
                    flush=True
                )
    ...
    async def enviar_mensagem(self, telefone, mensagem, nome_lead=""):
        ...
        if not self.conectado:
            print(
                f"[WA/Pili {_ts()}] ✗ WhatsApp não conectado",
                flush=True
            )
            return False
        ...
        if not numero:
            print(
                f"[WA/Pili {_ts()}] ✗ Número inválido: {telefone}",
                flush=True
            )
            return False
        ...
        print(
            f"[WA/Pili {_ts()}] ℹ Enviando para "
            f"{nome_display} ({numero})...",
            flush=True
        )
        ...
        try:
            ...
            if enviado:
                ...
                print(
                    f"[WA/Pili {_ts()}] ✓ Mensagem "
                    f"#{self.msgs_enviadas_hoje} enviada → {nome_display}",
                    flush=True
                )
                return True
            else:
                print(
                    f"[WA/Pili {_ts()}] ✗ Falha ao enviar para {numero}",
                    flush=True
                )
                return False
    ...
    async def verificar_respostas_pendentes(self, numeros: list) -> list:
        ...
        print(
            f"[WA/Pili {_ts()}] ℹ Verificando respostas em "
            f"{total} conversas...",
            flush=True
        )
        ...
        for i, numero in enumerate(numeros[:10], 1):
            ...
            try:
                ...
                if dados and dados.get('lead_respondeu'):
                    ...
                    print(
                        f"[WA/Pili {_ts()}] ✓ Resposta de {nome}: "
                        f"\"{msg}\"",
                        flush=True
                    )
                ...
                print(
                    f"[WA/Pili {_ts()}] ⚠ Erro ao verificar "
                    f"{numero_fmt}: {e}",
                    flush=True
                )
                continue
        ...
        if resultados:
            print(
                f"[WA/Pili {_ts()}] ✓ {len(resultados)} resposta(s) "
                f"encontrada(s) de {total} verificadas",
                flush=True
            )
        else:
            print(
                f"[WA/Pili {_ts()}] ℹ Nenhuma resposta pendente "
                f"({total} verificadas)",
                flush=True
            )
        return resultados
    ...
    async def _ler_conversa_atual(self) -> dict:
        ...
        try:
            ...
            print(
                f"[WA/Pili {_ts()}] ⚠ Erro ao ler conversa: {e}",
                flush=True
            )
            return None
    ...
    async def responder_conversa(self, mensagem):
        ...
        try:
            ...
            print(
                f"[WA/Pili {_ts()}] ✗ Erro ao responder: {e}",
                flush=True
            )
            return False
    ...
    async def delay_entre_mensagens(self):
        ...
        if (self.msgs_enviadas_sessao > 0
            ...
            print(
                f"[WA/Pili {_ts()}] ⚠ PAUSA LONGA {pausa:.0f}s "
            ...
            print(
                f"[WA/Pili {_ts()}] ℹ Aguardando {delay:.0f}s "
                "entre mensagens...",
                flush=True
            )
