...
async def ciclo_envio(
    ...
    for lead in leads:
        ...
        print(
            f"[WA/Pili {_ts()}] ℹ Contatando: {nome} "
...
async def ciclo_respostas(bot: WhatsAppBot, gerador: GeradorMensagens):
    ...
    if not numeros:
        print(
            f"[WA/Pili {_ts()}] ℹ Nenhuma empresa contactada para verificar",
            flush=True
        )
        return

    print(
        f"[WA/Pili {_ts()}] ℹ Verificando respostas: "
    ...
    print(
        f"[WA/Pili {_ts()}] ✓ {len(com_resposta)} resposta(s) "
        f"a processar",
        flush=True
    )
    for info in com_resposta:
        ...
        print(
            f"[WA/Pili {_ts()}] ℹ Respondendo {nome} "
            f"(estágio: {estagio})...",
            flush=True
        )
        ...
        if ok:
            ...
            if nivel in ('demo', 'agendamento'):
                ...
                print(
                    f"[WA/Pili {_ts()}] ✓ Demo proposta para {nome}",
                    flush=True
                )
            elif nivel == 'confirmacao':
                marcar_demo_confirmado(empresa['id'])
                print(
                    f"[WA/Pili {_ts()}] ✓ Demo CONFIRMADA: {nome}",
                    flush=True
                )
            log_acao(
...
async def main():
    ...
    print(
        f"[WA/Pili {_ts()}] ℹ === Bot WhatsApp Pili Equipamentos iniciando ===",
        flush=True
    )

    init_database()
    print(f"[WA/Pili {_ts()}] ℹ Banco de dados inicializado", flush=True)
    ...
    print(
        f"[WA/Pili {_ts()}] ✓ Conectado! Iniciando ciclos de prospecção...",
        flush=True
    )
    ...
    try:
        while True:
            ...
            if agora.hour < HORARIO_INICIO or agora.hour >= HORARIO_FIM:
                print(
                    f"[WA/Pili {_ts()}] ℹ Fora do horário "
            ...
            if agora.weekday() not in DIAS_ATIVOS:
                print(
                    f"[WA/Pili {_ts()}] ℹ Fim de semana. Aguardando 1h...",
                    flush=True
                )
        ...
        print(
            f"[WA/Pili {_ts()}] ℹ Bot encerrado pelo operador", flush=True
        )
    finally:
