...
class WhatsAppBot:
    ...
    async def iniciar(self):
        ...
        print(f"[WA/Prisma {_ts()}] ℹ Iniciando WhatsApp bot...", flush=True)
        ...
        print(
            f"[WA/Prisma {_ts()}] ℹ Abrindo WhatsApp Web...",
            flush=True
        )
        ...
        if not ja_conectado:
            print(
                f"[WA/Prisma {_ts()}] ⚠ Sessão não encontrada — "
            ...
            try:
                import os as _os2
                _qr = _os2.path.join(
                    _os2.path.dirname(_os2.path.abspath(__file__)), 'wa_qr.png'
                )
                _os2.remove(_qr)
            except OSError:
                pass
            ...
            print(
                f"[WA/Prisma {_ts()}] ✓ Sessão salva detectada",
                flush=True
            )
            conectado = True
        ...
        if conectado:
            self.conectado = True
            print(
                f"[WA/Prisma {_ts()}] ✓ WhatsApp conectado e pronto!",
                flush=True
            )
            return True
        else:
            print(
                f"[WA/Prisma {_ts()}] ✗ WhatsApp não conectou em 5 min",
                flush=True
            )
            return False
    ...
    async def _aguardar_conexao(self, timeout=180):
        ...
        for i in range(timeout):
            ...
            if i % 15 == 0 and i > 0:
                print(
                    f"[WA/Prisma {_ts()}] ℹ Aguardando QR Code... ({i}s)",
                    flush=True
                )
    ...
    async def enviar_mensagem(self, telefone, mensagem, nome_lead=""):
        ...
        if not self.conectado:
            print(
                f"[WA/Prisma {_ts()}] ✗ WhatsApp não conectado",
                flush=True
            )
            return False
        ...
        if not numero:
            print(
                f"[WA/Prisma {_ts()}] ✗ Número inválido: {telefone}",
                flush=True
            )
            return False

        nome_display = nome_lead[:30] if nome_lead else numero
        print(
            f"[WA/Prisma {_ts()}] ℹ Enviando para {nome_display} ({numero})...",
            flush=True
        )
        ...
        try:
            url = (
                ...
                f"?phone={numero}&text={quote(mensagem)}"
            )
            await self.page.goto(url, timeout=20000)
            await asyncio.sleep(random.uniform(4, 7))

            # Detecta popup de número inválido
            erro = await self.page.query_selector(
                'div[data-animate-modal-body="true"]'
            )
            if erro:
                texto_erro = await erro.inner_text()
                if ('não está no WhatsApp' in texto_erro
                        or 'invalid' in texto_erro.lower()):
                    print(
                        f"[WA/Prisma {_ts()}] ⚠ {numero} não está no WhatsApp",
                        flush=True
                    )
                    try:
                        btn_ok = await self.page.query_selector(
            ...
            if enviado:
                ...
                print(
                    f"[WA/Prisma {_ts()}] ✓ Mensagem #{self.msgs_enviadas_hoje}"
                    f" enviada → {nome_display}",
                    flush=True
                )
                return True
            else:
                print(
                    f"[WA/Prisma {_ts()}] ✗ Falha ao enviar para {numero}",
                    flush=True
                )
                return False
            ...
            print(
                f"[WA/Prisma {_ts()}] ✗ Erro WhatsApp: {e}",
                flush=True
            )
            return False
    ...
    async def verificar_respostas_pendentes(self, numeros: list) -> list:
        ...
        resultados = []
        total = len(numeros[:10])
        print(
            f"[WA/Prisma {_ts()}] ℹ Verificando respostas em "
            f"{total} conversas...",
            flush=True
        )

        for i, numero in enumerate(numeros[:10], 1):
            ...
            try:
                ...
                if dados and dados.get('lead_respondeu'):
                    nome = dados.get('nome', numero_fmt)
                    msg = dados.get('ultima_recebida', '')[:60]
                    print(
                        f"[WA/Prisma {_ts()}] ✓ Resposta de {nome}: "
                        f"\"{msg}\"",
                        flush=True
                    )
                ...
                print(
                    f"[WA/Prisma {_ts()}] ⚠ Erro ao verificar "
                    f"{numero_fmt}: {e}",
                    flush=True
                )
                continue
        ...
        if resultados:
            print(
                f"[WA/Prisma {_ts()}] ✓ {len(resultados)} resposta(s) "
                f"encontrada(s) de {total} verificadas",
                flush=True
            )
        else:
            print(
                f"[WA/Prisma {_ts()}] ℹ Nenhuma resposta pendente "
                f"({total} verificadas)",
                flush=True
            )
        return resultados
        ...
        try:
            ...
            msgs = await self.page.evaluate("""
                () => {
                    ...
                    const comAttr = document.querySelectorAll(
                        '[data-pre-plain-text]'
                    );
                    if (comAttr.length > 0) {
                        comAttr.forEach(el => {
                            ...
                            const texto = textEl
                                ...
                                : el.innerText.trim();
                            if (!texto) return;
                            const deMim = attr.includes('Você') ||
            ...
            ultimas = msgs[-10:]
            msgs_recebidas = [
                m['texto'] for m in ultimas if not m.get('deMim')
            ]
            msgs_enviadas = [
                m['texto'] for m in ultimas if m.get('deMim')
            ]

            ultima = ultimas[-1] if ultimas else None
            lead_respondeu = (
                ultima and not ultima.get('deMim')
                and len(msgs_recebidas) > 0
            )
            ...
            return {
                ...
                'ultima_recebida': (
                    msgs_recebidas[-1] if msgs_recebidas else None
                ),
            ...
            print(
                f"[WA/Prisma {_ts()}] ⚠ Erro ao ler conversa: {e}",
                flush=True
            )
            return None

    async def responder_conversa(self, mensagem):
        """Responde na conversa aberta atualmente"""
        try:
            input_box = await self._get_input_box()
            if not input_box:
                return False
            ...
            print(
                f"[WA/Prisma {_ts()}] ✗ Erro ao responder: {e}",
                flush=True
            )
            return False

    async def _get_input_box(self):
        """Retorna a caixa de texto do WhatsApp Web"""
        seletores = [
    ...
    async def delay_entre_mensagens(self):
        ...
        if (self.msgs_enviadas_sessao > 0
                and self.msgs_enviadas_sessao % PAUSA_LONGA_A_CADA == 0):
            pausa = random.uniform(PAUSA_LONGA_MIN, PAUSA_LONGA_MAX)
            print(
                f"[WA/Prisma {_ts()}] ⚠ PAUSA LONGA {pausa:.0f}s "
                f"(anti-ban — {self.msgs_enviadas_sessao} msgs na sessão)",
                flush=True
            )
            ...
            print(
                f"[WA/Prisma {_ts()}] ℹ Aguardando {delay:.0f}s "
                f"entre mensagens...",
                flush=True
            )
    ...
    def pode_enviar(self, dia_warmup=7):
        ...
        if self.msgs_enviadas_hoje >= limite:
            return False, (
                ...
                f"({self.msgs_enviadas_hoje}/{limite})"
            )
